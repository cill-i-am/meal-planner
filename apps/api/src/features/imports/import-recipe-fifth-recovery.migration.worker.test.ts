...
describe("recipe fifth recovery migration", () => {
  it("preserves a 0020 replay value and restores every cleanup fence", async () => {
    const migrationIndex = testEnv.TEST_MIGRATIONS.findIndex(
      ({ name }) => name === "0021_recipe_fifth_recovery.sql"
    );
    ...
    const importId = "00000000-0000-4000-8000-000000000219";
    ...
    await testEnv.MealPlannerDatabase.prepare(
      `INSERT INTO pilot_provider_budget_dispatches (
         ...
         'pilot-gaia-118', ?, 'retained-0020-replay', 'recipe-extraction',
    ...
    if (migration === undefined) {
      throw new Error("Missing recipe fifth recovery migration");
    }
    await applyD1Migrations(
      testEnv.MealPlannerDatabase,
      [migration],
    ...
    await expect(
      testEnv.MealPlannerDatabase.prepare(
        `SELECT name
           FROM sqlite_master
          WHERE type = 'table'
            AND name = 'pilot_provider_recipe_fifth_recoveries'`
      ).first()
    ).resolves.toEqual({
      name: "pilot_provider_recipe_fifth_recoveries",
    });
    await expect(
