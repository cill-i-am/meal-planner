...
const expectTerminal = (effect: Effect.Effect<unknown, unknown>) =>
  Effect.gen(function* expectTerminalEffect() {
    ...
    if (Exit.isSuccess(exit)) {
      return yield* Effect.die("Expected terminal media failure");
    }
...
describe("real media validation", () => {
  it.effect(
    "requires an ISO BMFF file-type box, audio, video, duration, and size",
    () =>
      Effect.gen(function* validateMediaProbeEffect() {
        const validated = yield* validateMediaProbe(validProbe, {
          actualBytes: 1024,
          maximumBytes: 2048,
  ...
  it.effect(
    "rejects filename/MIME-only, missing stream, corrupt, and bounded-limit cases",
    () =>
      Effect.all([
        expectTerminal(
          validateMediaProbe(
        ...
        expectTerminal(
          validateMediaProbe(
            {
              ...validProbe,
              format: { ...validProbe.format, duration: "901" },
            },
            {
              actualBytes: 1024,
              maximumBytes: 2048,
