...
describe("recipe eighth recovery migration", () => {
  it("preserves an existing replay and extends the immutable identity only through recovery:8", async () => {
    const migrationIndex = testEnv.TEST_MIGRATIONS.findIndex(
      ({ name }) => name === "0024_recipe_eighth_recovery.sql"
    );
    ...
    const importId = "00000000-0000-4000-8000-000000000247";
    ...
    if (migration === undefined) {
      throw new Error("Missing recipe eighth recovery migration");
    }
    await applyD1Migrations(
      testEnv.MealPlannerDatabase,
      [migration],
    ...
    expect(replayTable?.sql).toContain(":recovery:8");
    expect(replayTable?.sql).not.toContain(":recovery:9");
    await expect(
      testEnv.MealPlannerDatabase.prepare(
        `SELECT name
           FROM sqlite_master
    ...
    await expect(
      testEnv.MealPlannerDatabase.prepare(
        `SELECT name
           FROM sqlite_master
          WHERE type = 'table'
            AND name = 'pilot_provider_recipe_eighth_recoveries'`
      ).first()
    ).resolves.toEqual({
      name: "pilot_provider_recipe_eighth_recoveries",
    });
    await expect(
