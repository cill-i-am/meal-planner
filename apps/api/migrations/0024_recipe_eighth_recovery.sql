...
CREATE TABLE `pilot_provider_recipe_eighth_recoveries` (
  `runtime_stage` text NOT NULL,
  `import_id` text NOT NULL,
  ...
  `fifth_recovery_dispatch_id` text NOT NULL,
  `sixth_recovery_dispatch_id` text NOT NULL,
  `seventh_recovery_dispatch_id` text NOT NULL,
  `recovery_dispatch_id` text NOT NULL,
  `evidence_fingerprint` text NOT NULL,
  `seventh_recovery_extraction_fingerprint` text NOT NULL,
  ...
  CONSTRAINT `pilot_provider_recipe_eighth_recoveries_import_fk`
    FOREIGN KEY (`import_id`, `acquisition_generation`)
    REFERENCES `recipe_imports` (`id`, `acquisition_generation`)
    ON UPDATE RESTRICT ON DELETE RESTRICT,
  CONSTRAINT `pilot_provider_recipe_eighth_recoveries_seventh_recovery_fk`
    FOREIGN KEY (`runtime_stage`, `seventh_recovery_dispatch_id`)
    REFERENCES `pilot_provider_recipe_seventh_recoveries` (
      `runtime_stage`,
      `recovery_dispatch_id`
    )
    ON UPDATE RESTRICT ON DELETE RESTRICT,
  CONSTRAINT `pilot_provider_recipe_eighth_recoveries_seventh_extraction_fk`
    FOREIGN KEY (`seventh_recovery_extraction_fingerprint`)
    REFERENCES `import_recipe_extractions` (`extraction_fingerprint`)
    ON UPDATE RESTRICT ON DELETE RESTRICT,
  CONSTRAINT `pilot_provider_recipe_eighth_recoveries_stage_check`
    CHECK (`runtime_stage` = 'pilot-gaia-118'),
  CONSTRAINT `pilot_provider_recipe_eighth_recoveries_identity_check`
    CHECK (
      ...
      AND `fifth_recovery_dispatch_id` =
        `original_dispatch_id` || ':recovery:5'
      AND `sixth_recovery_dispatch_id` =
        `original_dispatch_id` || ':recovery:6'
      AND `seventh_recovery_dispatch_id` =
        `original_dispatch_id` || ':recovery:7'
      AND `recovery_dispatch_id` =
        `original_dispatch_id` || ':recovery:8'
      AND instr(`original_dispatch_id`, ':recovery:') = 0
      AND `seventh_recovery_extraction_fingerprint` <>
        `recovery_extraction_fingerprint`
      AND length(`evidence_fingerprint`) = 64
      AND `evidence_fingerprint` NOT GLOB '*[^0-9a-f]*'
      AND length(`seventh_recovery_extraction_fingerprint`) = 64
      AND `seventh_recovery_extraction_fingerprint`
        NOT GLOB '*[^0-9a-f]*'
...
CREATE UNIQUE INDEX
  `pilot_provider_recipe_eighth_recoveries_recovery_dispatch_unique`
  ON `pilot_provider_recipe_eighth_recoveries` (
    `runtime_stage`,
    `recovery_dispatch_id`
  );

CREATE UNIQUE INDEX
  `pilot_provider_recipe_eighth_recoveries_recovery_extraction_unique`
  ON `pilot_provider_recipe_eighth_recoveries` (
    `recovery_extraction_fingerprint`
  );

CREATE TRIGGER `pilot_provider_recipe_eighth_recoveries_immutable_update`
BEFORE UPDATE ON `pilot_provider_recipe_eighth_recoveries`
BEGIN
  SELECT RAISE(ABORT, 'pilot provider eighth recipe recovery is immutable');
END;

CREATE TRIGGER `pilot_provider_recipe_eighth_recoveries_immutable_delete`
BEFORE DELETE ON `pilot_provider_recipe_eighth_recoveries`
BEGIN
  SELECT RAISE(ABORT, 'pilot provider eighth recipe recovery is immutable');
END;
...
WHEN
  ...
  AND NEW.`dispatch_id` GLOB '*:recovery:[12345678]'
BEGIN
  SELECT CASE
...
CREATE TRIGGER `pilot_provider_recipe_eighth_recoveries_prepare`
AFTER INSERT ON `pilot_provider_recipe_eighth_recoveries`
BEGIN
  SELECT CASE
    WHEN NOT EXISTS (
      SELECT 1
        FROM `pilot_provider_recipe_seventh_recoveries` AS seventh_recovery
        JOIN `pilot_provider_recipe_sixth_recoveries` AS sixth_recovery
          ON sixth_recovery.`runtime_stage` =
               seventh_recovery.`runtime_stage`
         AND sixth_recovery.`import_id` = seventh_recovery.`import_id`
         AND sixth_recovery.`acquisition_generation` =
               seventh_recovery.`acquisition_generation`
         AND sixth_recovery.`recovery_dispatch_id` =
               seventh_recovery.`sixth_recovery_dispatch_id`
        JOIN `pilot_provider_recipe_fifth_recoveries` AS fifth_recovery
          ON fifth_recovery.`runtime_stage` =
               sixth_recovery.`runtime_stage`
         AND fifth_recovery.`import_id` = sixth_recovery.`import_id`
         AND fifth_recovery.`acquisition_generation` =
               sixth_recovery.`acquisition_generation`
         AND fifth_recovery.`recovery_dispatch_id` =
               sixth_recovery.`fifth_recovery_dispatch_id`
        JOIN `pilot_provider_recipe_fourth_recoveries` AS fourth_recovery
          ON fourth_recovery.`runtime_stage` =
               fifth_recovery.`runtime_stage`
        ...
        JOIN `pilot_provider_budget_reconciliations` AS audit
          ON audit.`runtime_stage` = seventh_recovery.`runtime_stage`
         AND audit.`dispatch_id` = seventh_recovery.`recovery_dispatch_id`
        ...
        JOIN `import_recipe_extractions` AS seventh_extraction
          ON seventh_extraction.`extraction_fingerprint` =
               NEW.`seventh_recovery_extraction_fingerprint`
         AND seventh_extraction.`import_id` = checkpoint.`import_id`
         AND seventh_extraction.`acquisition_generation` =
               checkpoint.`acquisition_generation`
         AND seventh_extraction.`evidence_fingerprint` =
               NEW.`evidence_fingerprint`
         AND seventh_extraction.`state` = 'failed'
         AND seventh_extraction.`failure_code` = 'provider_error'
         AND seventh_extraction.`is_current` = 0
       ...
       WHERE seventh_recovery.`runtime_stage` = NEW.`runtime_stage`
         AND seventh_recovery.`import_id` = NEW.`import_id`
         AND seventh_recovery.`acquisition_generation` =
               NEW.`acquisition_generation`
         AND seventh_recovery.`original_dispatch_id` =
               NEW.`original_dispatch_id`
         AND seventh_recovery.`first_recovery_dispatch_id` =
               NEW.`first_recovery_dispatch_id`
         AND seventh_recovery.`second_recovery_dispatch_id` =
               NEW.`second_recovery_dispatch_id`
         AND seventh_recovery.`third_recovery_dispatch_id` =
               NEW.`third_recovery_dispatch_id`
         AND seventh_recovery.`fourth_recovery_dispatch_id` =
               NEW.`fourth_recovery_dispatch_id`
         AND seventh_recovery.`fifth_recovery_dispatch_id` =
               NEW.`fifth_recovery_dispatch_id`
         AND seventh_recovery.`sixth_recovery_dispatch_id` =
               NEW.`sixth_recovery_dispatch_id`
         AND seventh_recovery.`recovery_dispatch_id` =
               NEW.`seventh_recovery_dispatch_id`
         AND seventh_recovery.`recovery_extraction_fingerprint` =
               NEW.`seventh_recovery_extraction_fingerprint`
         AND seventh_recovery.`evidence_fingerprint` =
               NEW.`evidence_fingerprint`
         AND seventh_recovery.`transcript_sha256` =
               NEW.`transcript_sha256`
         AND seventh_recovery.`visual_manifest_sha256` =
               NEW.`visual_manifest_sha256`
         AND seventh_recovery.`evidence_references_json` =
               NEW.`evidence_references_json`
         ...
         AND NOT EXISTS (
           SELECT 1
            ...
            WHERE replay.`runtime_stage` = seventh_recovery.`runtime_stage`
              AND replay.`dispatch_id` =
                    seventh_recovery.`recovery_dispatch_id`
         )
    )
    THEN RAISE(
      ABORT,
      'pilot provider eighth recipe recovery preconditions rejected'
    )
  END;
END;
