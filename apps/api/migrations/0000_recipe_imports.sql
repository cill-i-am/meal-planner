...
--> statement-breakpoint
CREATE TABLE IF NOT EXISTS "recipe_review_transitions" (
...
--> statement-breakpoint
CREATE INDEX `recipe_reviews_lifecycle_updated_index` ON `recipe_reviews` (`lifecycle`,`updated_at`);
--> statement-breakpoint
...
CREATE TABLE `recipe_review_mutations` (
	...
	`applied_at` text NOT NULL, `item_count` integer NOT NULL
...
--> statement-breakpoint
CREATE UNIQUE INDEX `recipe_review_mutations_review_version_unique` ON `recipe_review_mutations` (`extraction_fingerprint`,`resulting_version`);
--> statement-breakpoint
...
WHEN NEW.`command_kind` = 'correction'
  AND NOT EXISTS (
        SELECT 1
          FROM `import_recipe_extractions` AS `extraction`
...
BEFORE INSERT ON `recipe_review_mutations`
WHEN NEW.`command_kind` = 'transition'
  AND NOT EXISTS (
        SELECT 1
          FROM `import_recipe_extractions` AS `extraction`
          JOIN `recipe_imports` AS `intent`
