...
CREATE TABLE `pilot_provider_recipe_seventh_recoveries` (
  `runtime_stage` text NOT NULL,
  `import_id` text NOT NULL,
  ...
  `evidence_fingerprint` text NOT NULL,
  `sixth_recovery_extraction_fingerprint` text NOT NULL,
  ...
  CONSTRAINT `pilot_provider_recipe_seventh_recoveries_import_fk`
    FOREIGN KEY (`import_id`, `acquisition_generation`)
    REFERENCES `recipe_imports` (`id`, `acquisition_generation`)
    ON UPDATE RESTRICT ON DELETE RESTRICT,
  CONSTRAINT `pilot_provider_recipe_seventh_recoveries_sixth_recovery_fk`
    FOREIGN KEY (`runtime_stage`, `sixth_recovery_dispatch_id`)
    REFERENCES `pilot_provider_recipe_sixth_recoveries` (
      `runtime_stage`,
      `recovery_dispatch_id`
    )
    ON UPDATE RESTRICT ON DELETE RESTRICT,
  CONSTRAINT `pilot_provider_recipe_seventh_recoveries_sixth_extraction_fk`
    FOREIGN KEY (`sixth_recovery_extraction_fingerprint`)
    REFERENCES `import_recipe_extractions` (`extraction_fingerprint`)
    ON UPDATE RESTRICT ON DELETE RESTRICT,
  CONSTRAINT `pilot_provider_recipe_seventh_recoveries_stage_check`
    CHECK (`runtime_stage` = 'pilot-gaia-118'),
  CONSTRAINT `pilot_provider_recipe_seventh_recoveries_identity_check`
    CHECK (
      ...
      AND `sixth_recovery_extraction_fingerprint` <>
        `recovery_extraction_fingerprint`
      AND length(`evidence_fingerprint`) = 64
      AND `evidence_fingerprint` NOT GLOB '*[^0-9a-f]*'
      AND length(`sixth_recovery_extraction_fingerprint`) = 64
      AND `sixth_recovery_extraction_fingerprint`
        NOT GLOB '*[^0-9a-f]*'
...
CREATE UNIQUE INDEX
  `pilot_provider_recipe_seventh_recoveries_recovery_dispatch_unique`
  ON `pilot_provider_recipe_seventh_recoveries` (
    `runtime_stage`,
    `recovery_dispatch_id`
  );

CREATE UNIQUE INDEX
  `pilot_provider_recipe_seventh_recoveries_recovery_extraction_unique`
  ON `pilot_provider_recipe_seventh_recoveries` (
    `recovery_extraction_fingerprint`
  );

CREATE TRIGGER `pilot_provider_recipe_seventh_recoveries_immutable_update`
BEFORE UPDATE ON `pilot_provider_recipe_seventh_recoveries`
BEGIN
  SELECT RAISE(ABORT, 'pilot provider seventh recipe recovery is immutable');
END;

CREATE TRIGGER `pilot_provider_recipe_seventh_recoveries_immutable_delete`
BEFORE DELETE ON `pilot_provider_recipe_seventh_recoveries`
BEGIN
  SELECT RAISE(ABORT, 'pilot provider seventh recipe recovery is immutable');
END;
...
WHEN
  ...
  AND NEW.`dispatch_id` GLOB '*:recovery:[1234567]'
BEGIN
  SELECT CASE
...
CREATE TRIGGER `pilot_provider_recipe_seventh_recoveries_prepare`
AFTER INSERT ON `pilot_provider_recipe_seventh_recoveries`
BEGIN
  SELECT CASE
    WHEN NOT EXISTS (
      SELECT 1
        FROM `pilot_provider_recipe_sixth_recoveries` AS sixth_recovery
        ...
        JOIN `pilot_provider_budget_reconciliations` AS audit
          ON audit.`runtime_stage` = sixth_recovery.`runtime_stage`
         AND audit.`dispatch_id` = sixth_recovery.`recovery_dispatch_id`
        ...
        JOIN `import_recipe_extractions` AS sixth_extraction
          ON sixth_extraction.`extraction_fingerprint` =
               NEW.`sixth_recovery_extraction_fingerprint`
         AND sixth_extraction.`import_id` = checkpoint.`import_id`
         AND sixth_extraction.`acquisition_generation` =
               checkpoint.`acquisition_generation`
         AND sixth_extraction.`evidence_fingerprint` =
               NEW.`evidence_fingerprint`
         AND sixth_extraction.`state` = 'failed'
         AND sixth_extraction.`failure_code` = 'provider_error'
         AND sixth_extraction.`is_current` = 0
       ...
       WHERE sixth_recovery.`runtime_stage` = NEW.`runtime_stage`
         AND sixth_recovery.`import_id` = NEW.`import_id`
         AND sixth_recovery.`acquisition_generation` =
               NEW.`acquisition_generation`
         AND sixth_recovery.`original_dispatch_id` =
               NEW.`original_dispatch_id`
         AND sixth_recovery.`first_recovery_dispatch_id` =
               NEW.`first_recovery_dispatch_id`
         AND sixth_recovery.`second_recovery_dispatch_id` =
               NEW.`second_recovery_dispatch_id`
         AND sixth_recovery.`third_recovery_dispatch_id` =
               NEW.`third_recovery_dispatch_id`
         AND sixth_recovery.`fourth_recovery_dispatch_id` =
               NEW.`fourth_recovery_dispatch_id`
         AND sixth_recovery.`fifth_recovery_dispatch_id` =
               NEW.`fifth_recovery_dispatch_id`
         AND sixth_recovery.`recovery_dispatch_id` =
               NEW.`sixth_recovery_dispatch_id`
         AND sixth_recovery.`recovery_extraction_fingerprint` =
               NEW.`sixth_recovery_extraction_fingerprint`
         AND sixth_recovery.`evidence_fingerprint` =
               NEW.`evidence_fingerprint`
         AND sixth_recovery.`transcript_sha256` =
               NEW.`transcript_sha256`
         AND sixth_recovery.`visual_manifest_sha256` =
               NEW.`visual_manifest_sha256`
         AND sixth_recovery.`evidence_references_json` =
               NEW.`evidence_references_json`
         ...
         AND NOT EXISTS (
           SELECT 1
            ...
            WHERE replay.`runtime_stage` = sixth_recovery.`runtime_stage`
              AND replay.`dispatch_id` =
                    sixth_recovery.`recovery_dispatch_id`
         )
    )
    THEN RAISE(
      ABORT,
      'pilot provider seventh recipe recovery preconditions rejected'
    )
  END;
END;
