...
CREATE TABLE `pilot_provider_recipe_sixth_recoveries` (
  `runtime_stage` text NOT NULL,
  `import_id` text NOT NULL,
  ...
  `evidence_fingerprint` text NOT NULL,
  `fifth_recovery_extraction_fingerprint` text NOT NULL,
  ...
  CONSTRAINT `pilot_provider_recipe_sixth_recoveries_import_fk`
    FOREIGN KEY (`import_id`, `acquisition_generation`)
    REFERENCES `recipe_imports` (`id`, `acquisition_generation`)
    ON UPDATE RESTRICT ON DELETE RESTRICT,
  CONSTRAINT `pilot_provider_recipe_sixth_recoveries_fifth_recovery_fk`
    FOREIGN KEY (`runtime_stage`, `fifth_recovery_dispatch_id`)
    REFERENCES `pilot_provider_recipe_fifth_recoveries` (
      `runtime_stage`,
      `recovery_dispatch_id`
    )
    ON UPDATE RESTRICT ON DELETE RESTRICT,
  CONSTRAINT `pilot_provider_recipe_sixth_recoveries_fifth_extraction_fk`
    FOREIGN KEY (`fifth_recovery_extraction_fingerprint`)
    REFERENCES `import_recipe_extractions` (`extraction_fingerprint`)
    ON UPDATE RESTRICT ON DELETE RESTRICT,
  CONSTRAINT `pilot_provider_recipe_sixth_recoveries_stage_check`
    CHECK (`runtime_stage` = 'pilot-gaia-118'),
  CONSTRAINT `pilot_provider_recipe_sixth_recoveries_identity_check`
    CHECK (
      ...
      AND `fifth_recovery_extraction_fingerprint` <>
        `recovery_extraction_fingerprint`
      AND length(`evidence_fingerprint`) = 64
      AND `evidence_fingerprint` NOT GLOB '*[^0-9a-f]*'
      AND length(`fifth_recovery_extraction_fingerprint`) = 64
      AND `fifth_recovery_extraction_fingerprint`
        NOT GLOB '*[^0-9a-f]*'
...
CREATE UNIQUE INDEX
  `pilot_provider_recipe_sixth_recoveries_recovery_dispatch_unique`
  ON `pilot_provider_recipe_sixth_recoveries` (
    `runtime_stage`,
    `recovery_dispatch_id`
  );

CREATE UNIQUE INDEX
  `pilot_provider_recipe_sixth_recoveries_recovery_extraction_unique`
  ON `pilot_provider_recipe_sixth_recoveries` (
    `recovery_extraction_fingerprint`
  );

CREATE TRIGGER `pilot_provider_recipe_sixth_recoveries_immutable_update`
BEFORE UPDATE ON `pilot_provider_recipe_sixth_recoveries`
BEGIN
  SELECT RAISE(ABORT, 'pilot provider sixth recipe recovery is immutable');
END;

CREATE TRIGGER `pilot_provider_recipe_sixth_recoveries_immutable_delete`
BEFORE DELETE ON `pilot_provider_recipe_sixth_recoveries`
BEGIN
  SELECT RAISE(ABORT, 'pilot provider sixth recipe recovery is immutable');
END;
...
WHEN
  ...
  AND NEW.`dispatch_id` GLOB '*:recovery:[123456]'
BEGIN
  SELECT CASE
...
CREATE TRIGGER `pilot_provider_recipe_sixth_recoveries_prepare`
AFTER INSERT ON `pilot_provider_recipe_sixth_recoveries`
BEGIN
  SELECT CASE
    WHEN NOT EXISTS (
      SELECT 1
        ...
        JOIN `pilot_provider_budget_reconciliations` AS audit
          ON audit.`runtime_stage` = fifth_recovery.`runtime_stage`
         AND audit.`dispatch_id` = fifth_recovery.`recovery_dispatch_id`
        ...
        JOIN `import_recipe_extractions` AS fifth_extraction
          ON fifth_extraction.`extraction_fingerprint` =
               NEW.`fifth_recovery_extraction_fingerprint`
       ...
       WHERE fifth_recovery.`runtime_stage` = NEW.`runtime_stage`
         AND fifth_recovery.`import_id` = NEW.`import_id`
         ...
         AND fifth_recovery.`second_recovery_dispatch_id` =
               NEW.`second_recovery_dispatch_id`
         AND fifth_recovery.`third_recovery_dispatch_id` =
               NEW.`third_recovery_dispatch_id`
         AND fifth_recovery.`fourth_recovery_dispatch_id` =
               NEW.`fourth_recovery_dispatch_id`
         AND fifth_recovery.`recovery_dispatch_id` =
               NEW.`fifth_recovery_dispatch_id`
         AND fifth_recovery.`recovery_extraction_fingerprint` =
               NEW.`fifth_recovery_extraction_fingerprint`
         AND fifth_recovery.`evidence_fingerprint` =
               NEW.`evidence_fingerprint`
         AND fifth_recovery.`transcript_sha256` =
               NEW.`transcript_sha256`
         AND fifth_recovery.`visual_manifest_sha256` =
               NEW.`visual_manifest_sha256`
         AND fifth_recovery.`evidence_references_json` =
               NEW.`evidence_references_json`
    ...
    THEN RAISE(
      ABORT,
      'pilot provider sixth recipe recovery preconditions rejected'
    )
  END;
END;
