...
CREATE TABLE `pilot_provider_recipe_fourth_recoveries` (
  `runtime_stage` text NOT NULL,
  `import_id` text NOT NULL,
  ...
  `evidence_fingerprint` text NOT NULL,
  `third_recovery_extraction_fingerprint` text NOT NULL,
  ...
  CONSTRAINT `pilot_provider_recipe_fourth_recoveries_import_fk`
    FOREIGN KEY (`import_id`, `acquisition_generation`)
    REFERENCES `recipe_imports` (`id`, `acquisition_generation`)
    ON UPDATE RESTRICT ON DELETE RESTRICT,
  CONSTRAINT `pilot_provider_recipe_fourth_recoveries_third_recovery_fk`
    FOREIGN KEY (`runtime_stage`, `third_recovery_dispatch_id`)
    REFERENCES `pilot_provider_recipe_third_recoveries` (
      `runtime_stage`,
      `recovery_dispatch_id`
    )
    ON UPDATE RESTRICT ON DELETE RESTRICT,
  CONSTRAINT `pilot_provider_recipe_fourth_recoveries_third_extraction_fk`
    FOREIGN KEY (`third_recovery_extraction_fingerprint`)
    REFERENCES `import_recipe_extractions` (`extraction_fingerprint`)
    ON UPDATE RESTRICT ON DELETE RESTRICT,
  CONSTRAINT `pilot_provider_recipe_fourth_recoveries_stage_check`
    CHECK (`runtime_stage` = 'pilot-gaia-118'),
  CONSTRAINT `pilot_provider_recipe_fourth_recoveries_identity_check`
    CHECK (
      ...
      AND `third_recovery_extraction_fingerprint` <>
        `recovery_extraction_fingerprint`
      AND length(`evidence_fingerprint`) = 64
      AND `evidence_fingerprint` NOT GLOB '*[^0-9a-f]*'
      AND length(`third_recovery_extraction_fingerprint`) = 64
      AND `third_recovery_extraction_fingerprint`
        NOT GLOB '*[^0-9a-f]*'
...
CREATE UNIQUE INDEX
  `pilot_provider_recipe_fourth_recoveries_recovery_dispatch_unique`
  ON `pilot_provider_recipe_fourth_recoveries` (
    `runtime_stage`,
    `recovery_dispatch_id`
  );

CREATE UNIQUE INDEX
  `pilot_provider_recipe_fourth_recoveries_recovery_extraction_unique`
  ON `pilot_provider_recipe_fourth_recoveries` (
    `recovery_extraction_fingerprint`
  );

CREATE TRIGGER `pilot_provider_recipe_fourth_recoveries_immutable_update`
BEFORE UPDATE ON `pilot_provider_recipe_fourth_recoveries`
BEGIN
  SELECT RAISE(ABORT, 'pilot provider fourth recipe recovery is immutable');
END;

CREATE TRIGGER `pilot_provider_recipe_fourth_recoveries_immutable_delete`
BEFORE DELETE ON `pilot_provider_recipe_fourth_recoveries`
BEGIN
  SELECT RAISE(ABORT, 'pilot provider fourth recipe recovery is immutable');
END;
...
WHEN
  ...
  AND NEW.`dispatch_id` GLOB '*:recovery:[1234]'
BEGIN
  SELECT CASE
    WHEN NOT EXISTS (
      SELECT 1
...
CREATE TRIGGER `pilot_provider_recipe_fourth_recoveries_prepare`
AFTER INSERT ON `pilot_provider_recipe_fourth_recoveries`
BEGIN
  SELECT CASE
    WHEN NOT EXISTS (
      SELECT 1
        ...
        JOIN `pilot_provider_budget_reconciliations` AS audit
          ON audit.`runtime_stage` = third_recovery.`runtime_stage`
         AND audit.`dispatch_id` = third_recovery.`recovery_dispatch_id`
        ...
        JOIN `import_recipe_extractions` AS third_extraction
          ON third_extraction.`extraction_fingerprint` =
               NEW.`third_recovery_extraction_fingerprint`
       ...
       WHERE third_recovery.`runtime_stage` = NEW.`runtime_stage`
         AND third_recovery.`import_id` = NEW.`import_id`
         ...
         AND third_recovery.`second_recovery_dispatch_id` =
               NEW.`second_recovery_dispatch_id`
         AND third_recovery.`recovery_dispatch_id` =
               NEW.`third_recovery_dispatch_id`
         AND third_recovery.`recovery_extraction_fingerprint` =
               NEW.`third_recovery_extraction_fingerprint`
         AND third_recovery.`evidence_fingerprint` =
               NEW.`evidence_fingerprint`
         AND third_recovery.`transcript_sha256` =
               NEW.`transcript_sha256`
         AND third_recovery.`visual_manifest_sha256` =
               NEW.`visual_manifest_sha256`
         AND third_recovery.`evidence_references_json` =
               NEW.`evidence_references_json`
    ...
    THEN RAISE(
      ABORT,
      'pilot provider fourth recipe recovery preconditions rejected'
    )
  END;
END;
