...
CREATE TABLE `pilot_provider_recipe_second_recoveries` (
  `runtime_stage` text NOT NULL,
  `import_id` text NOT NULL,
  ...
  `evidence_fingerprint` text NOT NULL,
  `first_recovery_extraction_fingerprint` text NOT NULL,
  ...
  CONSTRAINT `pilot_provider_recipe_second_recoveries_import_fk`
    FOREIGN KEY (`import_id`, `acquisition_generation`)
    REFERENCES `recipe_imports` (`id`, `acquisition_generation`)
    ON UPDATE RESTRICT ON DELETE RESTRICT,
  CONSTRAINT `pilot_provider_recipe_second_recoveries_first_recovery_fk`
    FOREIGN KEY (`runtime_stage`, `first_recovery_dispatch_id`)
    REFERENCES `pilot_provider_recipe_recoveries` (
      `runtime_stage`,
      `recovery_dispatch_id`
    )
    ON UPDATE RESTRICT ON DELETE RESTRICT,
  CONSTRAINT `pilot_provider_recipe_second_recoveries_first_extraction_fk`
    FOREIGN KEY (`first_recovery_extraction_fingerprint`)
    REFERENCES `import_recipe_extractions` (`extraction_fingerprint`)
    ON UPDATE RESTRICT ON DELETE RESTRICT,
  CONSTRAINT `pilot_provider_recipe_second_recoveries_stage_check`
    CHECK (`runtime_stage` = 'pilot-gaia-118'),
  CONSTRAINT `pilot_provider_recipe_second_recoveries_identity_check`
    CHECK (
      ...
      AND `first_recovery_extraction_fingerprint` <>
        `recovery_extraction_fingerprint`
      AND length(`evidence_fingerprint`) = 64
      AND `evidence_fingerprint` NOT GLOB '*[^0-9a-f]*'
      AND length(`first_recovery_extraction_fingerprint`) = 64
      AND `first_recovery_extraction_fingerprint`
        NOT GLOB '*[^0-9a-f]*'
...
CREATE UNIQUE INDEX
  `pilot_provider_recipe_second_recoveries_recovery_dispatch_unique`
  ON `pilot_provider_recipe_second_recoveries` (
    `runtime_stage`,
    `recovery_dispatch_id`
  );

CREATE UNIQUE INDEX
  `pilot_provider_recipe_second_recoveries_recovery_extraction_unique`
  ON `pilot_provider_recipe_second_recoveries` (
    `recovery_extraction_fingerprint`
  );

CREATE TRIGGER `pilot_provider_recipe_second_recoveries_immutable_update`
BEFORE UPDATE ON `pilot_provider_recipe_second_recoveries`
BEGIN
  SELECT RAISE(ABORT, 'pilot provider second recipe recovery is immutable');
END;

CREATE TRIGGER `pilot_provider_recipe_second_recoveries_immutable_delete`
BEFORE DELETE ON `pilot_provider_recipe_second_recoveries`
BEGIN
  SELECT RAISE(ABORT, 'pilot provider second recipe recovery is immutable');
END;
...
WHEN
  ...
  AND NEW.`dispatch_id` GLOB '*:recovery:[12]'
BEGIN
  SELECT CASE
    WHEN NOT EXISTS (
      SELECT 1
...
CREATE TRIGGER `pilot_provider_recipe_second_recoveries_prepare`
AFTER INSERT ON `pilot_provider_recipe_second_recoveries`
BEGIN
  SELECT CASE
    WHEN NOT EXISTS (
      SELECT 1
        FROM `pilot_provider_recipe_recoveries` AS first_recovery
        ...
        JOIN `import_recipe_extractions` AS recovery_extraction
          ON recovery_extraction.`extraction_fingerprint` =
               NEW.`first_recovery_extraction_fingerprint`
         AND recovery_extraction.`import_id` = checkpoint.`import_id`
       ...
       WHERE first_recovery.`runtime_stage` = NEW.`runtime_stage`
         ...
         AND first_recovery.`recovery_extraction_fingerprint` =
               NEW.`first_recovery_extraction_fingerprint`
         AND first_recovery.`evidence_fingerprint` =
               NEW.`evidence_fingerprint`
         AND first_recovery.`transcript_sha256` =
               NEW.`transcript_sha256`
         AND first_recovery.`visual_manifest_sha256` =
               NEW.`visual_manifest_sha256`
         AND first_recovery.`evidence_references_json` =
               NEW.`evidence_references_json`
         ...
         AND NOT EXISTS (
           SELECT 1
            ...
            WHERE replay.`runtime_stage` = first_recovery.`runtime_stage`
    ...
    THEN RAISE(
      ABORT,
      'pilot provider second recipe recovery preconditions rejected'
    )
  END;
END;
