...
- Evidence retention: exactly seven days, with deletion verification required
  after the run.
- Source submission: URL-only for all six slots, through the authenticated
  import boundary.
...
| `sample_e20f7d94` | `carousel` | `carousel` | none | Typed `unsupported_carousel` acquisition failure; zero provider/model calls and zero provider spend |
| `sample_95ac31f8` | `expected_failure` | `video` | `expected_failure` | Typed `not_a_recipe` failure |

Prepared manifest ID: `manifest_3e7a9c52`.

Do not infer recipe quality, accessibility, or authorization from a candidate
label. Confirm each fact through the approved operator process before building
the executable manifest. The carousel slot deliberately ends when the existing
resolver identifies the unsupported source; it is not converted into a recipe
success path. If another source resolves to a different media kind or semantic
class, stop and replace the candidate or revise the sample design before
execution.
...
- [x] Every slot accepts only its authorized TikTok URL as source input.
- [x] Both video and carousel media kinds are represented.
- [x] The carousel slot expects `unsupported_carousel` with zero provider/model
  calls and zero provider spend.
