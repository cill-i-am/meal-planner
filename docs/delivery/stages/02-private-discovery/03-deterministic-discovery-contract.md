...
The maintained TanStack adapter owns streamed response parsing for both admitted models. The application preserves the forced Effect tool schema, Kimi thinking and sampling settings, configured token allowance and deadline, one-call dispatch fence, one gateway attempt, and disabled private logging/cache. It accepts at most 64 KiB of submitted tool arguments; the removed custom parser's framing and wire counters are not part of the new implementation. Streamed usage remains unknown and cannot reduce a budget reservation. Schema strictness cannot guarantee provider success or semantic truth; invalid output stays failed with the previous snapshot intact.
