...
Kimi uses the [documented SSE response](https://developers.cloudflare.com/workers-ai/models/kimi-k2.6/) with `stream: true` and `stream_options.include_usage: true`; GPT retains its JSON response. The application buffers the entire Kimi response within the existing 2 MiB and whole-operation deadline, including reasoning and SSE overhead. It decodes UTF-8 and complete SSE events, assembles the single choice's indexed tool deltas, and requires a terminal finish, `[DONE]` and end of the response before the existing tool/schema validation and native transaction. No partial message, card or continuity is published. ID, name and argument fragments follow [Moonshot's streaming tool guidance](https://github.com/MoonshotAI/Kimi-K2/blob/main/docs/tool_call_guidance.md#tool-calling-in-streaming-mode); the final function name must still match exactly. Consistent completed usage may arrive on the terminal choice or final usage event; missing usage remains unknown. Malformed, truncated, conflicting or extra terminal data rejects without a JSON fallback, repair or another call. This is a transport change; live hosted compatibility and semantic acceptance still require evidence, and historical failures remain unchanged.
