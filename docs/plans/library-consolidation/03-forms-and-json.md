...
Follow the existing [form procedure](../../how-to/build-a-form.md) and reference.
Compile the installed Effect `Schema.toStandardSchemaV1` adapter against TanStack
Form's actual validation contract. Define only the raw field shape needed by this
form, reusing canonical constraints where they fit rather than duplicating enums or
forcing a command schema onto unrelated UI fields.

Attach validation at the appropriate existing field/form events. Preserve touched,
blur/change and submit timing; show actionable field/form issues through accessible
associations, not a silently ignored submit or noisy errors before interaction.
Conditional fields validate for the selected variant, and irrelevant fields neither
block a valid command nor leak into one.

Keep explicit decoding/transformation in submission: successful Standard Schema
validation does not establish that `onSubmit` receives the transformed domain value.
Preserve `decodeFact`/`commandFor`, or an equally direct field-to-variant-to-command
boundary, including trimming and any earned domain/branded output. Authoritative
server validation remains. Domain conversion is not generic plumbing to delete.

Keep safety consent, provisional meaning and current-version authority separate
from field validity. Kind/target or proposed-meaning changes retire obsolete consent;
no-known-constraints and safety reductions retain the existing explicit confirmation.
Preserve ordinary edit and removal behavior. Reuse validation for private-card
correction only when meaning and shared-file ownership match; private proposals and
Household writes keep distinct commands and confirmation boundaries.

A visible draft can change while an older command remains unresolved, but its
revalidation never replaces the original dispatched payload, mutation ID, versions
or binding. Keep pending/disabled behavior and caller-owned recovery intact. Remove
only redundant validation/error-state mechanics; do not add another schema/form
library or a generic form-generation layer.
...
- [ ] New preferences, hard constraints, no-known-constraints, ordinary edits and
  removals preserve the existing domain outcomes. Kind changes cannot let hidden
  fields bypass validation, block an otherwise valid variant or enter its command.
- [ ] Invalid then corrected input produces useful field/form errors at the expected
  touched/blur/change/submit times. Labels, issue associations, focus, keyboard submit
  and disabled/loading controls work in an actual browser using synthetic profiles.
- [ ] Whitespace, normalization and transformed/branded values are explicitly decoded
  before command construction. The submitted command retains the old meaning;
  validation-only success is not mistaken for an earned domain value.
- [ ] Safety reduction and no-known-constraint confirmation remain explicit; kind,
  target or proposed-meaning changes cannot retain stale consent. Provisional facts,
  current review/version checks and private-card boundaries retain their meaning.
- [ ] During an unresolved command, draft edits, refresh and revalidation cannot
  replace its payload/ID/versions or dispatch a forbidden sibling action. Delayed
  callbacks cannot clear a newer pending intent or imply an unproven result.
- [ ] Actual installed schema/form integration passes affected form/panel tests and
  the production web build. Reused correction validation is exercised at both
  consumers; server validation and explicit domain conversion remain intact.
- [ ] The removal inventory identifies redundant validation/error-state mechanics,
  not renamed domain functions. No additional form/schema framework, unused dependency
  or duplicate mutable status record remains; architecture assessment stays separate.
