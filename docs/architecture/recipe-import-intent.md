...
guarded aggregate transitions and a fenced execution generation. The existing
review ledger and approved recipe bank remain authoritative children. Workflow
state, review data, and recipe data are not copied into a second public model.
...
public version, and appends `source_resolved` history.
...
Processing stages are monotonic. `resolving_source` requires a pending source;
every later stage requires a resolved safe source and preserves the stage's
original `startedAt`. Safe activity is `working` or `retrying`, with an optional
`nextAttemptAt`. Heartbeats and executor attempts are private and do not change
the public version. Meaningful stage, activity, action, recovery, or terminal
changes do.
...
`failed` never resumes and exposes only the recovery choice
`create_new_intent`, `contact_support`, or `none`. Cancellation is legal only
from `processing` or `requires_action`. A later confirmation slice will insert
one immutable household-owned recipe, associate it with the intent, and succeed
without shadowing review authority.
...
receive one truthful `migration_snapshot`, not fabricated stages. Workflow
generation fencing and a scheduled D1-guarded stalled-work reconciler remain
execution responsibilities for later slices; polling reads stay pure.
...
cancellation handlers, timeline handlers, recipe-result handlers, provider
operations, and cloud deployment are deferred. Durable intent/action/recipe
metadata lives in D1; private evidence remains in short-retention R2. A future
batch resource may group independently addressable intent IDs without changing
