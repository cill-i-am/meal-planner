...
If `origin/main` advances before edit authority, hold work and notify the orchestrator. Use only the non-destructive exact-base refresh in `worktree-isolation`; then rerun relevant baselines and repeat the plan/reviewer gate. Do not reset, rebase speculatively, or continue from stale handoff state.
